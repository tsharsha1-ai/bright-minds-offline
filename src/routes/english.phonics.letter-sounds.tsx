...
export const Route = createFileRoute("/english/phonics/letter-sounds")({
  component: LetterSoundsPage,
});
...
function LetterSoundsPage() {
  const { mark } = useProgress();
  const [i, setI] = useState(0);
  useEffect(() => { mark("en.phonics.letters", "visited"); }, [mark]);
  const p = PHONICS[i];

  return (
    <>
      <TopBar title="Letter Sounds" />
      <PageShell>
        <motion.div
          key={i}
        ...
        <div className="mt-8 flex justify-center gap-3">
          ...
          <button
            onClick={() => {
              if (i === PHONICS.length - 1) mark("en.phonics.letters", "practiced");
